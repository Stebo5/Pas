...
print(
    "[%s] UDP Calc Server is waiting for incoming connections ... " % strftime(
        "%Y-%m-%d %H:%M:%S", gmtime()))
...
try:
    while True:
        ...
        if data1 and data2 and op:
            ...
            print(
                "[%s] Got from client %s ... : %s %s %s" % (
                    strftime("%Y-%m-%d %H:%M:%S", gmtime()), str(address),
                    data1, op,
                    data2))
            ...
            try:
                ...
                if op == '+':
                    ...
                    result = "Bad operator. I support only +, -, *, / math " \
                             "operators"
                    sent = sock.sendto(str(result).encode('utf-8'), address)
